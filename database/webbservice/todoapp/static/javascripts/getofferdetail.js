...
var OfferBox = React.createClass({
  loadOffersFromServer: function() {
    $.ajax({
      ...
      error: function(xhr, status, err, errorThrown) {
        console.error(this.props.url, status, err.toString(), errorThrown);
      }.bind(this)
    });
  },
  ...
  render: function() {
    /*var offerNodes = this.props.data.map(function(offer, index) {
      return (
        ...
        <Offer title={offer.title} user_id={offer.user_id} location={offer.location} category={offer.category} subcategory={offer.subcategory} created_at={offer.created_at} updated_at={offer.updated_at} key={index}>
          {offer.title}
        </Offer>
      );
    });*/
...
React.render(
  <OfferBox url={urlPath} pollInterval={2000} />,
